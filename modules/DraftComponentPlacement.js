...
export class DraftComponentPlacement {
    ...
    begin(componentDefinition) {
        ...
        this.ghost = this.createGhostMesh(componentDefinition);
    ...
    createGhostMesh(componentDefinition) {
        ...
        mesh.traverse(child => {
            if (child.isMesh) {
                child.material = this.validMaterial.clone();
            }
        });
        return mesh;
    }
    ...
    applyGridSnap(position) {
        position.x =
            ...
            this.gridSize;
        position.z =
            Math.round(position.z /this.gridSize) *
            this.gridSize;
    }
    ...
    place() {
        if (!this.isValidPlacement)
            return;
        const placed = this.currentComponent.mesh.clone();
        placed.position.copy(this.ghost.position);
    ...
    cancel() {
        if (this.ghost) {
            this.editor.scene.remove(this.ghost);
        }
        this.enabled = false;
        this.ghost = null;
        this.currentComponent = null;
